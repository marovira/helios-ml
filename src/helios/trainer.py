...
class _CheckpointKeys(enum.StrEnum):
    """Defines the top-level keys for saved checkpoints."""
...
class Trainer:
    ...
    def _validate_state_dict(self, state_dict: dict[str, typing.Any]) -> bool:
        ...
        chkpt_ver = pv.Version(state_dict[_CheckpointKeys.VERSION])
    ...
    def _save_checkpoint(self, state: TrainingState) -> None:
        ...
        state_dict[_CheckpointKeys.VERSION] = __version__
        state_dict[_CheckpointKeys.TRAINING_STATE] = state
    ...
    def _load_checkpoint(
        ...
        loggers.restore_loggers(
            self._run_name, self._log_root, state_dict.get(_CheckpointKeys.LOGGERS, {})
        )
        if not skip_rng:
        ...
        return state_dict[_CheckpointKeys.TRAINING_STATE]
